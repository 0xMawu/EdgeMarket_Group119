...
export const AccountCard = memo(function AccountCard({ account, isFollowing, onToggleFollow, expanded, onToggleExpand, rank }: Props) {
  ...
  const truncatedCount =
    (account.totalOpenPositionCount ?? account.openPositions.length) - account.openPositions.length;
  ...
  return (
    <View style={styles.card}>
      ...
      {account.openPositions.length > 0 && (
        <Pressable style={styles.expandBtn} onPress={() => onToggleExpand(account.address)}>
      ...
      {expanded && (
        <View style={styles.positionsList}>
          {account.openPositions.map((pos) => (
            <PositionCard key={pos.id} position={pos} targetAddress={account.address} />
          ))}
          {truncatedCount > 0 && (
            <Text style={styles.truncatedLabel}>
              +{truncatedCount} more with Premium
            </Text>
          )}
        </View>
      )}
