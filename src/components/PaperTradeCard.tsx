 ...
 *
 * Shows a summary header with total unrealised P&L, then one row per copied
 * trade. Individual positions are copied via the Copy button on each
 * PositionCard — this component is read-only.
 */
  ...
  return (
    <View style={styles.tradeRow}>
      <View style={styles.tradeFlex}>
        ...
        <Text style={styles.tradeTarget}>via {shortAddr(trade.targetAddress)}</Text>
      </View>
...
export function PaperTradeCard({ portfolio, loading, error, onRefresh }: Props) {
...
const styles = StyleSheet.create({
  ...
  tradeTarget: { color: colors.textFainter, fontSize: 10, marginTop: 2 },
